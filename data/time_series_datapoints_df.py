...
with modifications by ryan yost 
...
def get_wto_data(indicator: str, reportEcon: list, ps_query_string: str, key: str) -> pd.DataFrame:
    ...
    for econ in reportEcon.code:
        ...
        try:
            ...
            ok = ok + 1
            okList.append(econ)
            print(econ,"-", "ok", ok, "-", total, ": ", indicator)

        except:
            failList.append(econ)
            fail = fail +1
            print(econ,"-", "failed", fail, "-",total, ": ", indicator)

    return pd.concat(listOfData)
...
def get_time_series_datapoints(indicator_code,  # strings only
    ...
    data = pd.concat([pd.pivot_table(pd.DataFrame.from_dict(data, orient='index').reset_index(), columns='index', aggfunc='first') for data in returnedData['Dataset']], axis = 0).reset_index(drop=True)
